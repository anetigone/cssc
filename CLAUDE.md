...
- **Phase 8** ⬜ **进行中**：成本感知搜索。8.0 ✅ `CostVector` 观测；8.1 ✅ branch/obligation 成本归因；8.2 ✅ opt-in cost-aware frontier（默认 legacy 排序不变，`--frontier-policy cost_aware_v1` opt-in，readiness 仍是 gate，`branch_id` 最终 tie-breaker）；8.3+ ⬜ 软预算、value-per-cost、真实任务消融。方案见 [tmp/phase8_plan.md](tmp/phase8_plan.md)，成本策略只改 structured 调度，不改证明语义。
