...
live runner 也已完成：同一 suite 进程复用一个强制 persistent Lean server，每题保持独立
controller、预算、trace 和 result，支持配置哈希校验、断点续跑和仅重跑基础设施失败；
proof tool 与正式 checker 复用该 server，checker 基础设施失败不会静默退回逐候选冷启动，
provider generation failure 也不计入证明失败；trace 提供 failure-first pretty reader，并在
provider failure 时保留此前 tool call 的状态与耗时。两题真实 server pilot 已通过；模型、arm、task ids 和重复次数的
正式实验冻结仍未完成。
