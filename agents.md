...
- 8.4 ✅ 价值/成本混合评分：`FrontierPolicy.VALUE_PER_COST_V1`（`_value_per_cost_priority_key`，全整数 tuple 实现 `unlock_value * progress_likelihood * information_gain / expected_incremental_cost`，不做浮点除法——value 维度分组在前、cost 在后），stalled/overdraft gate 仍前置（继承 V2）。`FrontierNode` 新增 `unlock_value`/`progress_likelihood`（最新 attempt 改变 goal-fingerprint 集合）/`information_gain`（capability probe 0/1）三字段，legacy/v1/v2 key 忽略。每次 `pop()` 产 `PriorityExplanation`（`final_key_or_score` 是 int 序列），所有 policy 都记录；`metadata["priority_explanations"]` 透传所有 result 路径。`--frontier-policy` choices 补全为全四档。
